...
same, which is false against the artifact — §2.19 settled: the refused build
...
namespace — was itself overturned by §2.19: the binding trace shows every one
...
### 2.19 "Weak-definition coalescing makes an ABI-implementing MPI unwrappable on macOS"
...
identity-configuration capture (§2.3, whose mechanism §2.19 later corrected to
