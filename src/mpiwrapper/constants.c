/* libmpiwrapper -- the ABI <-> implementation constant tables.
 *
 ...
 *
 *  - Case labels are *numeric*, with the ABI's symbolic name in a comment.
 *    MPIABI_INT expands to ((MPIABI_Datatype)0x00000209), and casting an
 ...
 *    the header, so nothing is transcribed either way.
 *
 ...
 *    standard makes optional (the sized Fortran types, the error classes newer
 *    than MPI-3.1, MPI_T) carry an #ifdef.
 *
 ...
 * implementation a fabricated handle -- on MPICH an int whose kind bits it will
 * happily interpret. Returning the class's null handle instead makes the
 ...
 * unrelated to the ABI's: the ABI's RDONLY is 16 where both implementations use
 * 2, so this is a decomposition rather than a switch (NOTES.md #5.5).
 *
 * There are *two* mappers rather than the one NOTES.md #5.5 described, and Open
 * MPI is why. The ABI puts file modes (1..256) and window asserts
 ...
 * which. (MPICH keeps the two families disjoint, so a single mapper round-trips
 * there and the bug would have shipped.) This is the same shape as ranks and
 * tags: the role is a property of the parameter, so it belongs in the function
 * name, and apis.json is what tells the generator which to emit.
 */
...
/* Sentinels. MPI_BOTTOM is (void *)0 in the ABI and in both implementations, so
 * that arm is an identity today -- it is emitted anyway, because the ABI fixes
 * the value and an implementation is free not to, and a site that omits the
 * test is invisible until that implementation appears. MPI_IN_PLACE is not
