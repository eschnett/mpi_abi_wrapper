...
#      direction?
#
#   2. CAPTURE -- this project's load-time isolation. Coalescing is decided
#      among the images that export weakly, and the first loaded wins; an
#      all-strong libmpi_abi never enters it. If we became weak to satisfy (1),
#      who wins then? (HISTORY.md #2.19 and dev/weakdef-probe/ for the
#      mechanism, and for why #2.3's capture was a different one.)
#
...
# because it did not run cleanly, and its motivation is gone now that #2.19 has
# the real mechanism; README.md "What this does not settle" has what it showed,
# rather than a broken section left in the file.
